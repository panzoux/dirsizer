...
- [ ] Enumerators, not done: the worker sweep for a winner (none qualified), a fallback from B to `find` (needed only if B becomes the default), cold cache, ReFS and network shares (a share root was not tried), a second machine.
