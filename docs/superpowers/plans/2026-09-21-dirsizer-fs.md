...
| M5 | `src\DirSizer.Fs\Walker.cs` | in `WorkQueue.Finish`, delete the line `_peakQueued = Math.Max(_peakQueued, _stack.Count);` (the same line is also in `Seed`; delete only the one in `Finish`, for example by including the preceding `_pending += children.Count - 1;` line in the text you match) | `walk finishes on a very wide fan-out and reports the queue peak` |
