...
- [x] Add a separate `DirSizer.Bulk.csproj` / `DirSizer.Bulk.exe` experimental reader.
...
Current product state: `--reader=fsctl` is the default, `--reader=bulk` is opt-in and experimental, and there is no automatic fallback. Bulk should become a default candidate only after all of these:
...
- [ ] Failure paths that could not be executed here: no elevation, a failing raw read, a corrupt extent map.
