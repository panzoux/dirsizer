...
static class RecordComparison
{
    ...
    public static int Run(string volume, int maxSlots)
    {
        ...
        foreach (var pair in bulkFirst)
        {
            if (fsctl.TryGetValue(pair.Key, out var other)) RecordDiff.CompareRecord(pair.Key, pair.Value, other, tally);
...
static class CompareSelfTests
{
    ...
    {
        var tally = new Tally();
        RecordDiff.CompareRecord(7, pair.Left, pair.Right, tally);
